...
BASE_HEAD = "4c9fb0da8aad0e56030c5a69a3b06b9263fbba23"
UPSTREAM_COMMIT = "72033b8ec288aa067ce4530ff9d96bf231e9c4e5"
CAPABILITY_ID = "cap_56358709cbc328e25ed1afff"
EVIDENCE_ID = "EVID-W02-PARITY-BINDING-SERVER-CHAT-COMPLETIONS-20260924"
DECISION_ID = "DEC-W02-G6-SERVER-CHAT-COMPLETIONS-BINDING-20260924"
TEST_ID = (
    "tests.test_cp03_w02_parity_graph.W02ParityGraphTests."
    "test_server_chat_completions_binding_is_capability_specific_and_source_backed"
)
CLAIM_ID = "CLAIM-CP03-W02-PARITY-GRAPH-20260922"
CLAIM_EVENT_ID = "CLAIM-CONTINUE-W02-G6-SERVER-CHAT-COMPLETIONS-20260924"
WORK_STARTED_EVENT_ID = "RUN-W02-G6-SERVER-CHAT-COMPLETIONS-WORK-STARTED-20260924"
RUN_EVENT_ID = "RUN-W02-G6-SERVER-CHAT-COMPLETIONS-20260924"
WAVE_EVENT_ID = "WAVE-W02-G6-SERVER-CHAT-COMPLETIONS-20260924"
RISK_EVENT_ID = "RISK-UPDATE-W02-G6-K47-BINDINGS-SERVER-CHAT-COMPLETIONS-20260924"
...
def verify_base() -> None:
    ...
    assert "35/47" in state and "12` UNBOUND" in state
    ...
    assert claims[-1].get("wave_id") == "CP03-W02-PARITY-GRAPH-20260922"


def prove_red() -> None:
    result = run([sys.executable, "-m", "unittest", "-v", TEST_ID], expect_success=False)
    assert result.returncode != 0, "server chat-completions test unexpectedly existed/passed before mutation"


def register_work_started() -> None:
    append_jsonl_once(
        ROOT / "ledgers/RUN_LOG.ndjson",
        {
            "branch": "wave/cp03/w02-parity-server-chat-completions-20260924",
            "canonical_task": "W02-17",
            "checkpoint": "CP03",
            "claim_id": CLAIM_ID,
            "date": "2026-09-24",
            "event": "WORK_STARTED",
            "event_id": WORK_STARTED_EVENT_ID,
...
def build_source_probe() -> dict:
    ...
    return {
        ...
        "w02_source_facts": [
            "chat_completions_route_declared",
            "engine_agent_model_resolved",
            "stream_tools_branch_present",
            "stream_agent_branch_present",
            "stream_direct_branch_present",
            "nonstream_agent_branch_present",
            "nonstream_direct_branch_present",
            "completed_exchange_memory_hook_present",
        ],
    }
...
def patch_binding_and_tests(probe: dict) -> None:
    ...
    replace_once(
        ...
        'LEARNED_ROUTER_CAPABILITY_ID = "cap_cc6d3df603b0ebde91766a13"\nSERVER_CHAT_COMPLETIONS_CAPABILITY_ID = "cap_56358709cbc328e25ed1afff"\n',
    )
    replace_once(
        tests,
        'LEARNED_ROUTER_TEST_ID = (\n    "tests.test_cp03_w02_parity_graph.W02ParityGraphTests."\n    "test_learned_router_binding_is_capability_specific_and_source_backed"\n)\n',
        'LEARNED_ROUTER_TEST_ID = (\n    "tests.test_cp03_w02_parity_graph.W02ParityGraphTests."\n    "test_learned_router_binding_is_capability_specific_and_source_backed"\n)\nSERVER_CHAT_COMPLETIONS_TEST_ID = (\n    "tests.test_cp03_w02_parity_graph.W02ParityGraphTests."\n    "test_server_chat_completions_binding_is_capability_specific_and_source_backed"\n)\n',
    )
    replace_once(
        tests,
        "    def test_current_matrix_has_thirty_five_candidates_and_no_parity_promotion(self) -> None:\n",
        "    def test_current_matrix_has_thirty_six_candidates_and_no_parity_promotion(self) -> None:\n",
    )
    replace_once(
        tests,
        ...
        '        self.assertEqual(sum(row["w02_evidence_state"] == "EVIDENCE_BACKED_CANDIDATE" for row in rows), 36)\n        self.assertEqual(sum(row["w02_evidence_state"] == "UNBOUND" for row in rows), 11)\n',
    )
    replace_once(
        tests,
        ...
        "HEURISTIC_ROUTER_CAPABILITY_ID, VLLM_PEARL_MINING_CAPABILITY_ID, CLI_CHAT_COMMAND_CAPABILITY_ID, LEARNED_ROUTER_CAPABILITY_ID, SERVER_CHAT_COMPLETIONS_CAPABILITY_ID},",
    )

    insertion_marker = "    def test_ollama_registry_binding_is_capability_specific_and_evidence_backed(self) -> None:\n"
    method = '''    def test_server_chat_completions_binding_is_capability_specific_and_source_backed(self) -> None:
        row = next(row for row in self.result["rows"] if row["capability_id"] == SERVER_CHAT_COMPLETIONS_CAPABILITY_ID)
        ...
        self.assertEqual(row["source_line"], 159)
        ...
        self.assertEqual(binding["test_id"], SERVER_CHAT_COMPLETIONS_TEST_ID)
        ...
        probe = json.loads((ROOT / "evidence/cp03/cp03-w02/W02-17/binding_server_chat_completions_source_probe.json").read_text(encoding="utf-8"))
        self.assertEqual(probe["capability_id"], SERVER_CHAT_COMPLETIONS_CAPABILITY_ID)
        self.assertEqual(probe["source_line"], 159)
...
'''
    ...
    if "def test_server_chat_completions_binding_is_capability_specific_and_source_backed" not in text:
        if insertion_marker not in text:
            raise RuntimeError("server chat-completions test insertion marker missing")
        tests.write_text(text.replace(insertion_marker, method + insertion_marker, 1), encoding="utf-8")

    write_json(ROOT / "evidence/cp03/cp03-w02/W02-17/binding_server_chat_completions_source_probe.json", probe)


def persist(probe: dict) -> None:
    probe_path = "evidence/cp03/cp03-w02/W02-17/binding_server_chat_completions_source_probe.json"
    report = {
        "agent_execution": "NOT_RUN",
        ...
        "parity_promotions": 0,
        "path": "evidence/cp03/cp03-w02/W02-17/binding_server_chat_completions_report.json",
        "project_id": "CLEVER-JARVIS-001",
        "provider_egress_executions": 0,
    ...
    write_json(ROOT / "evidence/cp03/cp03-w02/W02-17/binding_server_chat_completions_report.json", report)
    ...
    append_jsonl_once(
        ROOT / "ledgers/CLAIM_LEDGER.ndjson",
        {
            ...
            "checkpoint": "CP03",
            "claim_id": CLAIM_ID,
            "coordination": "Continue the sole active W02-17 G6 claim for the branch-free SHARED core server chat-completions source-proof unit only. Existing AFM, Apple-FM, cloud-reload, InferenceStartEvent and server-delete-model scopes remain excluded. No route/agent/model/provider/tool execution, parity promotion, denominator mutation or W02-18 transition is authorized.",
            "date": "2026-09-24",
            "event": "CONTINUE",
    ...
    append_jsonl_once(
        ROOT / "ledgers/RISK_LEDGER.ndjson",
        {
            ...
            "evidence_id": EVIDENCE_ID,
            "mitigation": "Keep W02-17 IN_PROGRESS, VERIFIED parity at 0 and W02-18 BLOCKED. Static core-server route proof does not prove request execution, agent behavior, streaming behavior, tool semantics or cross-facet parity; continue one non-conflicting proof unit at a time.",
    ...
    replace_once(
        ...
        "G6 explicit binding progress: `36/47` evidence-backed candidates; `11` UNBOUND",
    )
    append_section_once(
        ROOT / "HANDOFF.md",
        "## W02-17 server chat-completions binding — source-backed partial verified",
        f'''## W02-17 server chat-completions binding — source-backed partial verified
