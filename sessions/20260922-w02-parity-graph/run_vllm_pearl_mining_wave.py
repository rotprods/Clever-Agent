...
BASE_HEAD = "5ff6d3223e2d2f2c7377b446878f4355dd4f9cd3"
UPSTREAM_COMMIT = "72033b8ec288aa067ce4530ff9d96bf231e9c4e5"
CAPABILITY_ID = "cap_1ec0c952d6796991c21dc790"
EVIDENCE_ID = "EVID-W02-PARITY-BINDING-VLLM-PEARL-MINING-20260924"
DECISION_ID = "DEC-W02-G6-VLLM-PEARL-MINING-BINDING-20260924"
TEST_ID = (
...
CLAIM_EVENT_ID = "CLAIM-CONTINUE-W02-G6-VLLM-PEARL-MINING-20260924"
RUN_EVENT_ID = "RUN-W02-G6-VLLM-PEARL-MINING-20260924"
WAVE_EVENT_ID = "WAVE-W02-G6-VLLM-PEARL-MINING-20260924"
RISK_EVENT_ID = "RISK-UPDATE-W02-G6-K47-BINDINGS-VLLM-PEARL-MINING-20260924"
SOURCE_EVIDENCE_ID = "EVID-W02-MODEL-BRIDGE-20260921"
SOURCE_EVIDENCE_HEAD = "a866c335a0f9cad75122c8eb7c5310d358f6aad4"
SOURCE_PATH = "src/openjarvis/engine/_discovery.py"
SOURCE_LINE = 119
SOURCE_BLOB_SHA1 = "047f7f1eea4e6cc60453a3e97d9d76689c85c6fb"
BINDINGS_BEFORE = 32
BINDINGS_AFTER = 33
UNBOUND_BEFORE = 15
UNBOUND_AFTER = 14
...
def verify_base() -> None:
    ...
    assert "32/47" in state and "15` UNBOUND" in state
...
def build_source_probe() -> dict:
    ...
    blob = subprocess.check_output(
        ["git", "-C", "/tmp/openjarvis", "hash-object", SOURCE_PATH], text=True
    ).strip()
    ...
    assert lines[SOURCE_LINE - 1].strip() == 'EngineRegistry.register_value("vllm-pearl-mining", _cls)'
    return {
        "canonical_parity_status": "UNVERIFIED",
        "capability_id": CAPABILITY_ID,
        "capability_name": "vllm-pearl-mining",
        "checkpoint": "CP03",
        "date": "2026-09-24",
        "dynamic_registration_execution": "NOT_RUN",
        "mining_sidecar_execution": "NOT_RUN",
        "model_executions": 0,
        "ownership": "OWNED",
        "parity_promotions": 0,
        ...
        "w02_source_facts": [
            "mining_sidecar_guard",
            "idempotent_registry_contains_guard",
            "sidecar_shape_gate",
            "vllm_endpoint_and_model_required",
            "dynamic_openai_compatible_engine_class",
            "vllm_pearl_mining_register_value",
        ],
    }
...
def patch_binding_and_tests(probe: dict) -> None:
    ...
    replace_once(
        ...
        "SERVER_MODEL_PULL_CAPABILITY_ID, SERVER_INFO_CAPABILITY_ID, HEURISTIC_ROUTER_CAPABILITY_ID, VLLM_PEARL_MINING_CAPABILITY_ID},",
    )

    marker = "    def test_heuristic_router_binding_is_capability_specific_and_source_backed(self) -> None:\n"
    method = '''    def test_vllm_pearl_mining_binding_is_capability_specific_and_source_backed(self) -> None:
        row = next(row for row in self.result["rows"] if row["capability_id"] == VLLM_PEARL_MINING_CAPABILITY_ID)
        self.assertEqual(row["ownership"], "OWNED")
        self.assertEqual(row["surface_kind"], "registry_registration")
        self.assertEqual(row["source_path"], "src/openjarvis/engine/_discovery.py")
        self.assertEqual(row["source_line"], 119)
        self.assertEqual(row["name"], "vllm-pearl-mining")
        ...
        self.assertEqual(binding["test_id"], VLLM_PEARL_MINING_TEST_ID)
        ...
        probe = json.loads((ROOT / "evidence/cp03/cp03-w02/W02-17/binding_vllm_pearl_mining_source_probe.json").read_text(encoding="utf-8"))
        self.assertEqual(probe["capability_id"], VLLM_PEARL_MINING_CAPABILITY_ID)
        self.assertEqual(probe["source_line"], 119)
        self.assertEqual(probe["source_probe"], "PASS")
        self.assertFalse(probe["source_execution"])
        self.assertEqual(probe["dynamic_registration_execution"], "NOT_RUN")
        self.assertEqual(probe["mining_sidecar_execution"], "NOT_RUN")
...
'''
    ...
    if "def test_vllm_pearl_mining_binding_is_capability_specific_and_source_backed" not in text:
        if marker not in text:
            raise RuntimeError("vllm-pearl-mining test insertion marker missing")
        tests.write_text(text.replace(marker, method + marker, 1), encoding="utf-8")

    write_json(
        ROOT / "evidence/cp03/cp03-w02/W02-17/binding_vllm_pearl_mining_source_probe.json",
        probe,
    )


def persist(probe: dict) -> None:
    probe_path = "evidence/cp03/cp03-w02/W02-17/binding_vllm_pearl_mining_source_probe.json"
    probe_sha256 = hashlib.sha256((ROOT / probe_path).read_bytes()).hexdigest()
    report = {
        "binding_capability_id": CAPABILITY_ID,
        "binding_capability_name": "vllm-pearl-mining",
        ...
        "mining_sidecar_execution": "NOT_RUN",
        "model_executions": 0,
        "next_slice": "Continue W02-17 with one non-conflicting proof unit only when capability-specific execution or exact pinned-source evidence can be paired with an exact completed-task PASS receipt. Preserve AFM/fallback NOT_RUN and avoid overlapping branch scopes.",
        "openjarvis_obligations": 646,
        "parity_promotions": 0,
        "path": "evidence/cp03/cp03-w02/W02-17/binding_vllm_pearl_mining_report.json",
        "project_id": "CLEVER-JARVIS-001",
        "provider_egress_executions": 0,
    ...
    write_json(ROOT / "evidence/cp03/cp03-w02/W02-17/binding_vllm_pearl_mining_report.json", report)
    ...
    append_jsonl_once(
        ROOT / "ledgers/CLAIM_LEDGER.ndjson",
        {
            ...
            "checkpoint": "CP03",
            "claim_id": CLAIM_ID,
            "coordination": "Continue the sole active W02-17 G6 claim for the branch-free vllm-pearl-mining conditional registry source-proof unit only. No dynamic sidecar execution, model/provider/tool execution, parity promotion, denominator mutation, W02-18 transition, or overlap with AFM/Apple-FM/cloud-reload/InferenceStartEvent/server-delete-model scopes is authorized.",
            "date": "2026-09-24",
            "event": "CONTINUE",
    ...
    append_jsonl_once(
        ...
        {
            "checkpoint": "CP03",
            "context": "Exact pinned OpenJarvis source proves a conditional, idempotent vllm-pearl-mining registry path gated by mining sidecar shape. The completed W02-08 receipt is reused only as the exact G6 binding receipt; sidecar loading, dynamic class registration and engine behavior are NOT_RUN. This is source-backed candidate evidence, not behavioral parity.",
            "date": "2026-09-24",
            "decision": "Bind the frozen vllm-pearl-mining registry-registration surface as a non-terminal EVIDENCE_BACKED_CANDIDATE only.",
    ...
    append_jsonl_once(
        ROOT / "ledgers/RISK_LEDGER.ndjson",
        {
            ...
            "evidence_id": EVIDENCE_ID,
            "mitigation": "Keep W02-17 IN_PROGRESS, VERIFIED parity at 0 and W02-18 BLOCKED. Conditional source registration does not prove mining-sidecar execution, vLLM reachability, inference behavior or failover semantics; continue proof units independently.",
    ...
    replace_once(
        ...
        "G6 explicit binding progress: `33/47` evidence-backed candidates; `14` UNBOUND",
    )
    append_section_once(
        ROOT / "HANDOFF.md",
        "## W02-17 vllm-pearl-mining binding — source-backed partial verified",
        f'''## W02-17 vllm-pearl-mining binding — source-backed partial verified

- `{EVIDENCE_ID}`: `{CAPABILITY_ID}` (`vllm-pearl-mining`, OWNED at `{SOURCE_PATH}:{SOURCE_LINE}`) is now a non-terminal `EVIDENCE_BACKED_CANDIDATE`.
- Exact pinned OpenJarvis `{UPSTREAM_COMMIT}` proves only the conditional/idempotent source path: a mining sidecar must expose both `vllm_endpoint` and `model`, then a derived `_OpenAICompatibleEngine` class is registered as `vllm-pearl-mining`. Sidecar read, dynamic registration, engine health and inference remain `NOT_RUN`.
- Binding is anchored to completed W02-08 `{SOURCE_EVIDENCE_ID}` at exact SHA `{SOURCE_EVIDENCE_HEAD}`. Candidate evidence only: VERIFIED parity remains 0.
