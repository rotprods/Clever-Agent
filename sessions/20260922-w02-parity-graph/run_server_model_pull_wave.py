...
BASE_HEAD = "2e6aaeebab85293c5458faa9e0216f0a1c98b8ec"
UPSTREAM_COMMIT = "72033b8ec288aa067ce4530ff9d96bf231e9c4e5"
CAPABILITY_ID = "cap_abafd7c735464aaecfe109d7"
EVIDENCE_ID = "EVID-W02-PARITY-BINDING-SERVER-MODEL-PULL-20260924"
DECISION_ID = "DEC-W02-G6-SERVER-MODEL-PULL-BINDING-20260924"
TEST_ID = (
...
CLAIM_EVENT_ID = "CLAIM-CONTINUE-W02-G6-SERVER-MODEL-PULL-20260924"
RUN_EVENT_ID = "RUN-W02-G6-SERVER-MODEL-PULL-20260924"
WAVE_EVENT_ID = "WAVE-W02-G6-SERVER-MODEL-PULL-20260924"
RISK_EVENT_ID = "RISK-UPDATE-W02-G6-K47-BINDINGS-SERVER-MODEL-PULL-20260924"
...
def verify_base() -> None:
    ...
    assert "27/47" in state and "20` UNBOUND" in state
...
def build_source_probe() -> dict:
    ...
    return {
        ...
        "capability_name": "POST /v1/models/pull",
        "checkpoint": "CP03",
        "date": "2026-09-24",
        "http_method": "POST",
        "http_path": "/v1/models/pull",
        "input_model_required": True,
        "ollama_engine_gate_present": True,
        "ollama_api_path": "/api/pull",
        "ollama_pull_execution": "NOT_RUN",
        "http_listener_execution": "NOT_RUN",
        "model_download_execution": "NOT_RUN",
        "model_executions": 0,
        "ownership": "OWNED",
        "parity_promotions": 0,
...
def persist(probe: dict) -> None:
    ...
    report = {
        ...
        "parity_promotions": 0,
        "path": "evidence/cp03/cp03-w02/W02-17/binding_server_model_pull_report.json",
        "project_id": "CLEVER-JARVIS-001",
        "provider_egress_executions": 0,
    ...
    write_json(ROOT / "evidence/cp03/cp03-w02/W02-17/binding_server_model_pull_report.json", report)
    ...
    append_jsonl_once(ROOT / "ledgers/CLAIM_LEDGER.ndjson", {
        ...
        "checkpoint": "CP03",
        "claim_id": CLAIM_ID,
        "coordination": "Continue the sole active W02-17 G6 claim for the branch-free core server POST /v1/models/pull proof unit only. No model download, Ollama pull, provider/tool side effect, parity promotion, denominator mutation, or W02-18 transition is authorized.",
        "date": "2026-09-24",
        "event": "CONTINUE",
    ...
    append_jsonl_once(ROOT / "ledgers/DECISION_LEDGER.ndjson", {
        "checkpoint": "CP03",
        "context": "Exact pinned source proves the core POST /v1/models/pull route, required model input, Ollama-only gate and /api/pull request shape. W02-08 provides the exact model/engine bridge receipt, but HTTP listener and actual model download remain NOT_RUN; this evidence cannot promote parity.",
        "date": "2026-09-24",
        "decision": "Bind the frozen OpenJarvis POST /v1/models/pull route as a non-terminal EVIDENCE_BACKED_CANDIDATE only.",
    ...
    append_jsonl_once(ROOT / "ledgers/RISK_LEDGER.ndjson", {
        ...
        "evidence_id": EVIDENCE_ID,
        "mitigation": "Keep W02-17 IN_PROGRESS, verified parity at 0 and W02-18 BLOCKED. This slice proves route/source shape only; model download and Ollama side effects stay NOT_RUN. Continue one capability-specific proof unit at a time.",
    ...
    replace_once(
        ROOT / "STATE.md",
        "G6 explicit binding progress: `27/47` evidence-backed candidates; `20` UNBOUND",
    ...
    append_section_once(
        ...
        f'''## W02-17 core server POST /v1/models/pull binding — source-backed partial verified

- `{EVIDENCE_ID}`: `{CAPABILITY_ID}` (`POST /v1/models/pull`, OWNED route at `src/openjarvis/server/routes.py:1151`) is now a non-terminal `EVIDENCE_BACKED_CANDIDATE`.
- Exact pinned source `{UPSTREAM_COMMIT}` proves required model input, Ollama-only engine gating, bounded `/api/pull` request shape and explicit HTTP error mapping. HTTP listener, model download and Ollama pull execution remain `NOT_RUN`.
