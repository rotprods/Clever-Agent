...
BASE_HEAD = "07d26a0dcff9e3892ea51989f6dee8a002eeff0f"
UPSTREAM_COMMIT = "72033b8ec288aa067ce4530ff9d96bf231e9c4e5"
CAPABILITY_ID = "cap_fc0aa6591af55f5edc7f3a40"
EVIDENCE_ID = "EVID-W02-PARITY-BINDING-CLI-CHAT-COMMAND-20260924"
DECISION_ID = "DEC-W02-G6-CLI-CHAT-COMMAND-BINDING-20260924"
TEST_ID = (
...
CLAIM_EVENT_ID = "CLAIM-CONTINUE-W02-G6-CLI-CHAT-COMMAND-20260924"
RUN_EVENT_ID = "RUN-W02-G6-CLI-CHAT-COMMAND-20260924"
WAVE_EVENT_ID = "WAVE-W02-G6-CLI-CHAT-COMMAND-20260924"
RISK_EVENT_ID = "RISK-UPDATE-W02-G6-K47-BINDINGS-CLI-CHAT-COMMAND-20260924"
...
SOURCE_LINE = 82
SOURCE_BLOB_SHA1 = "7c767f8985bb62e190621f48b7f49ed51914b09b"
BINDINGS_BEFORE = 33
BINDINGS_AFTER = 34
UNBOUND_BEFORE = 14
UNBOUND_AFTER = 13
...
def verify_base() -> None:
    ...
    assert "33/47" in state and "14` UNBOUND" in state
    ...
    assert row["surface_kind"] == "cli_command"
...
def prove_red() -> None:
    ...
    assert result.returncode != 0, "new CLI chat capability test unexpectedly existed/passed before mutation"
...
def build_source_probe() -> dict:
    ...
    assert lines[SOURCE_LINE - 1].strip() == "def chat("
    return {
        ...
        "date": "2026-09-24",
        "interactive_chat_execution": "NOT_RUN",
...
def patch_binding_and_tests(probe: dict) -> None:
    ...
    replace_once(
        tests,
        "HEURISTIC_ROUTER_CAPABILITY_ID, VLLM_PEARL_MINING_CAPABILITY_ID},",
    ...
    method = '''    def test_cli_chat_command_binding_is_capability_specific_and_source_backed(self) -> None:
...
'''
    ...
    if "def test_cli_chat_command_binding_is_capability_specific_and_source_backed" not in text:
        if insertion_marker not in text:
            raise RuntimeError("CLI chat test insertion marker missing")
        tests.write_text(text.replace(insertion_marker, method + insertion_marker, 1), encoding="utf-8")

    write_json(
        ROOT / "evidence/cp03/cp03-w02/W02-17/binding_cli_chat_command_source_probe.json",
        probe,
    )


def persist(probe: dict) -> None:
    probe_path = "evidence/cp03/cp03-w02/W02-17/binding_cli_chat_command_source_probe.json"
    probe_sha256 = hashlib.sha256((ROOT / probe_path).read_bytes()).hexdigest()
    report = {
        "binding_capability_id": CAPABILITY_ID,
        "binding_capability_name": "chat",
        ...
        "interactive_chat_execution": "NOT_RUN",
        "model_executions": 0,
        "next_slice": "Continue W02-17 with one non-conflicting proof unit only when capability-specific execution or exact pinned-source evidence can be paired with an exact completed-task PASS receipt. Preserve AFM/fallback NOT_RUN and avoid extant Apple-FM/cloud-reload/InferenceStartEvent/server-delete-model scopes.",
        "openjarvis_obligations": 646,
        "parity_promotions": 0,
        "path": "evidence/cp03/cp03-w02/W02-17/binding_cli_chat_command_report.json",
        "project_id": "CLEVER-JARVIS-001",
        "provider_egress_executions": 0,
    ...
    write_json(ROOT / "evidence/cp03/cp03-w02/W02-17/binding_cli_chat_command_report.json", report)
    ...
    append_jsonl_once(
        ROOT / "ledgers/CLAIM_LEDGER.ndjson",
        {
            ...
            "checkpoint": "CP03",
            "claim_id": CLAIM_ID,
            "coordination": "Continue the sole active W02-17 G6 claim for the branch-free shared CLI chat source-proof unit only. No CLI execution, interactive chat execution, model/provider/tool execution, parity promotion, denominator mutation, W02-18 transition, or overlap with extant Apple-FM/cloud-reload/InferenceStartEvent/server-delete-model scopes is authorized.",
            "date": "2026-09-24",
            "event": "CONTINUE",
    ...
    append_jsonl_once(
        ...
        {
            "checkpoint": "CP03",
            "context": "Exact pinned OpenJarvis source proves the shared CLI chat declaration plus W02-relevant model registration, engine selection and chat-model resolution path. The completed W02-08 receipt is reused only as the exact G6 binding receipt. CLI execution, interactive chat, inference, provider egress and tools remain NOT_RUN. Shared W02 ownership forbids terminal attribution.",
            "date": "2026-09-24",
            "decision": "Bind the frozen CLI chat command as a non-terminal EVIDENCE_BACKED_CANDIDATE only.",
    ...
    append_jsonl_once(
        ROOT / "ledgers/RISK_LEDGER.ndjson",
        {
            ...
            "evidence_id": EVIDENCE_ID,
            "mitigation": "Keep W02-17 IN_PROGRESS, VERIFIED parity at 0 and W02-18 BLOCKED. Source structure does not prove interactive chat execution, inference behavior, memory/tool behavior or cross-facet parity; continue proof units independently.",
    ...
    append_section_once(
        ...
        f'''## W02-17 CLI chat binding — source-backed partial verified

- `{EVIDENCE_ID}`: `{CAPABILITY_ID}` (`chat`, SHARED W02/W03/W04/W05 CLI command at `{SOURCE_PATH}:{SOURCE_LINE}`) is now a non-terminal `EVIDENCE_BACKED_CANDIDATE`.
- Exact pinned OpenJarvis `{UPSTREAM_COMMIT}` proves the command declaration plus the W02-relevant `register_builtin_models()`, `get_engine(...)` and chat-model resolution source path. CLI execution, interactive chat, inference, memory/tool behavior and provider egress remain `NOT_RUN`.
- Binding is anchored to completed W02-08 `{SOURCE_EVIDENCE_ID}` at exact SHA `{SOURCE_EVIDENCE_HEAD}`. Because this proof unit is SHARED and terminal-ineligible in W02, no terminal attribution or parity promotion is made.
