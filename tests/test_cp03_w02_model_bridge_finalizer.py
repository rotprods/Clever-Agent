...
from scripts.cp03.finalize_w02_model_bridge import (
    CLAIM_ID,
    DECISION_ID,
    EVIDENCE_ID,
    RISK_ID,
...
class W02ModelBridgeFinalizerTests(unittest.TestCase):
    def fixture(self, root: Path) -> Path:
        ...
        write_json(
            ...
            "iterations/03/waves/CP03-W02/TASK_GRAPH.json",
            {
                "global_denominator": 7565,
                "openjarvis_obligations": 646,
                "first_executable_task": "W02-08",
                "tasks": [
                    {"id": "W02-06", "status": "COMPLETE", "proof": [{"result": "PASS"}]},
                    {"id": "W02-07", "status": "COMPLETE", "proof": [{"result": "PASS"}]},
                    {"id": "W02-08", "status": "READY", "proof": []},
                    {"id": "W02-09", "status": "BLOCKED", "proof": []},
                    {"id": "W02-10", "status": "BLOCKED", "proof": []},
                ],
            },
        )
        ...
        catalog = root / "native_catalog.json"
        write_json(
            root,
            "native_catalog.json",
            {
                ...
                "engine_count": 1,
                "model_count": 2,
                "engines": [{"key": "engine", "state": "REGISTERED"}],
                "models": [
                    {"key": "model-a", "state": "REGISTERED"},
                    {"key": "model-b", "state": "REGISTERED"},
                ],
                "import_failures": [],
    ...
    def test_finalize_advances_only_w02_08_and_is_idempotent(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            catalog = self.fixture(root)
            head = "a" * 40
            first = finalize(root=root, run_id=321, validated_head=head, catalog_path=catalog)
            self.assertEqual(first["completed_task"], "W02-08")
            self.assertEqual(first["next_task"], "W02-09")
            self.assertEqual(first["parity_promotions"], 0)
            self.assertEqual(first["native_engine_count"], 1)
            self.assertEqual(first["native_model_count"], 2)

            graph = json.loads((root / "iterations/03/waves/CP03-W02/TASK_GRAPH.json").read_text())
            tasks = {item["id"]: item for item in graph["tasks"]}
            self.assertEqual(graph["first_executable_task"], "W02-09")
            self.assertEqual(tasks["W02-08"]["status"], "COMPLETE")
            self.assertEqual(tasks["W02-08"]["proof"][0]["evidence_id"], EVIDENCE_ID)
            self.assertEqual(tasks["W02-09"]["status"], "READY")
            self.assertEqual(tasks["W02-10"]["status"], "BLOCKED")
            ...
            report = json.loads((root / "evidence/cp03/cp03-w02/W02-08/report.json").read_text())
            ...
            self.assertIn("W02-09", (root / "HANDOFF.md").read_text())

            second = finalize(root=root, run_id=321, validated_head=head, catalog_path=catalog)
    ...
    def test_finalize_refuses_parity_drift(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(RuntimeError, "parity drift"):
                finalize(root=root, run_id=1, validated_head="b" * 40, catalog_path=catalog)

    def test_finalize_refuses_to_skip_ahead_to_w02_10(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            catalog = self.fixture(root)
            graph_path = root / "iterations/03/waves/CP03-W02/TASK_GRAPH.json"
            graph = json.loads(graph_path.read_text())
            next(item for item in graph["tasks"] if item["id"] == "W02-10")["status"] = "READY"
            write_json(root, "iterations/03/waves/CP03-W02/TASK_GRAPH.json", graph)
            with self.assertRaisesRegex(RuntimeError, "W02-10"):
                finalize(root=root, run_id=1, validated_head="c" * 40, catalog_path=catalog)

    def test_finalize_refuses_catalog_that_executes_model_or_promotes_parity(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            catalog = self.fixture(root)
            data = json.loads(catalog.read_text())
            data["model_executions"] = 1
            data["parity_promotions"] = 1
            catalog.write_text(json.dumps(data), encoding="utf-8")
            with self.assertRaisesRegex(RuntimeError, "forbidden counter"):
                finalize(root=root, run_id=1, validated_head="d" * 40, catalog_path=catalog)
