...
class W02ParityGraphTests(unittest.TestCase):
    ...
    def test_current_matrix_has_ten_candidates_and_no_parity_promotion(self) -> None:
        ...
        self.assertEqual(sum(row["w02_evidence_state"] == "EVIDENCE_BACKED_CANDIDATE" for row in rows), 10)
        self.assertEqual(sum(row["w02_evidence_state"] == "UNBOUND" for row in rows), 37)
        ...
        self.assertEqual(
            set(candidates),
            {SEED_CAPABILITY_ID, OLLAMA_CAPABILITY_ID, LITELLM_CAPABILITY_ID, CLOUD_CAPABILITY_ID, NIM_CAPABILITY_ID, GEMMA_CPP_CAPABILITY_ID, OPENAI_COMPAT_REGISTER_CAPABILITY_ID, MODEL_REGISTER_VALUE_CAPABILITY_ID, MODEL_REGISTER_VALUE_1088_CAPABILITY_ID, CLI_MODEL_LIST_REGISTER_BUILTIN_CAPABILITY_ID},
        )
