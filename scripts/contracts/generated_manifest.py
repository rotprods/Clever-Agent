...
TRANSIENT_PARTS = frozenset({"__pycache__", ".cache", ".pytest_cache"})
...
def is_canonical_generated_file(path: Path) -> bool:
    """Return whether a generated artifact is stable source/wire output.

    Runtime caches and platform metadata are deliberately excluded from the
    reproducibility manifest. They are execution by-products, not generated SDK
    authority, and may vary across Python/macOS/toolchain runs.
    """
...
def build() -> dict[str, object]:
    ...
    for root in OUTPUTS:
        ...
        for path in sorted(item for item in absolute.rglob("*") if item.is_file()):
            if not is_canonical_generated_file(path.relative_to(ROOT)):
                continue
            files.append(
                {
...
def main() -> int:
    ...
    if args.check:
        ...
        print(f"OK: {payload['generated_file_count']} canonical generated files match manifest")
        return 0
    path.write_text(expected, encoding="utf-8")
    print(f"Wrote {MANIFEST} for {payload['generated_file_count']} canonical generated files")
    return 0
