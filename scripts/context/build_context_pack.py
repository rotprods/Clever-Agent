...
AUTHORITY_READ_ORDER = ["GOAL.md", "SECURITY_MODEL.md", "AGENTS.md", "ARCHITECTURE.md", "CAPABILITY_PARITY.md", "docs/COS_GRAPH_ENGINE_V2.md", "docs/GRAPH_ENGINEERING_PROTOCOL.md", "CHECKPOINT_REGISTRY.json", "STATE.md", "GOAL_STATE.json", "EXECUTION_STATE.json", "iterations/01/STATE.json", ".agentic/context/CURRENT_CONTEXT.json", "HANDOFF.md"]
...
GRAPH_PLANES = [
    ...
    {"id": "P1_SEMANTIC_SURFACE", "authority": "DERIVED_CANDIDATES", "mutable_by_higher_planes": False, "products": ["Graphify V2 semantic_surface_projection", "W03 registered/executable behavioral surfaces"]},
    ...
    {"id": "P3_AGENT_CONTEXT", "authority": "DERIVED_RECOVERY_VIEW", "mutable_by_higher_planes": False, "products": ["CURRENT_CONTEXT.json", "CURRENT_CONTEXT.md", "HANDOFF pointers"]},
]
...
def build_context_pack(root: Path = ROOT) -> dict[str, Any]:
    ...
    iteration = _load_json(root, "iterations/01/STATE.json")
    dimensions = _load_json(root, str(DIMENSIONS_JSON))
    ...
    active_claims = [{"claim_id": row["claim_id"], "wave_id": row.get("wave_id"), "owner": row.get("owner"), "status": row.get("status")} for row in claims.values() if row.get("status") == "ACTIVE"]
    open_risks = [{"risk_id": row["risk_id"], "severity": row.get("severity"), "status": row.get("status")} for row in risks.values() if row.get("status") == "OPEN"]
    ...
    pack = {
        ...
        "frontier": {"checkpoint": execution["current_checkpoint"], "iteration": execution["active_iteration"], "subcheckpoint": execution["active_subcheckpoint"], "next_wave": execution["next_wave"], "next_wave_name": execution["next_wave_name"], "completed_waves": execution.get("completed_waves", []), "parity_denominator_status": iteration.get("parity_denominator_status"), "blocking_issues": execution.get("blocking_issues", [])},
        ...
        "hard_invariants": {"chat_is_authority": False, "raw_graph_is_not_parity_denominator": True, "candidate_is_not_capability": True, "source_graph_is_immutable": True, "cos_decisions_are_provisional_until_promoted": True, "automatic_destructive_merge_forbidden": True, "migration_requires_behavioral_equivalence_evidence": True, "context_pack_is_derived_not_primary_truth": True, "max_context_loss_interactions": 1},
        ...
        "ledger_pointers": {"claims": "ledgers/CLAIM_LEDGER.ndjson", "risks": "ledgers/RISK_LEDGER.ndjson", "decisions": "ledgers/DECISION_LEDGER.ndjson", "evidence": "ledgers/EVIDENCE_LEDGER.ndjson", "capabilities": "ledgers/CAPABILITY_LEDGER.jsonl"},
        "required_next_outputs": execution.get("required_next_outputs", []),
        "recovery_commands": ["python scripts/validate_agentic_state.py", "python scripts/context/validate_context_pack.py", "python scripts/context/build_context_pack.py --check", "/empezarproyecto"],
    }
...
def render_markdown(pack: dict[str, Any]) -> str:
    ...
    lines = ["# CURRENT CONTEXT — Clever-Agent", "", "> Deterministic derived recovery view. It is never a primary source of truth; Git, canonical state and evidence outrank it.", "", "## Frontier", "", f"- Project: `{pack['project']['project_id']}`", f"- Checkpoint: `{frontier['checkpoint']}`", f"- Iteration: `{frontier['iteration']}` / `{frontier['subcheckpoint']}`", f"- Next wave: `{frontier['next_wave']} — {frontier['next_wave_name']}`", f"- Parity denominator: `{frontier['parity_denominator_status']}`", f"- Protocol: `{pack['protocol']}`", f"- 20D model: `{pack['dimension_model']['id']}` ({pack['dimension_model']['count']} dimensions)", "", "## Hard invariants", ""]
    ...
    for row in pack["open_risks"]:
        lines.append(f"- `{row['risk_id']}` · `{row.get('severity')}` · detail in `ledgers/RISK_LEDGER.ndjson`")
