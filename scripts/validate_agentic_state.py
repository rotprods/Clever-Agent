#!/usr/bin/env python3
"""Validate Clever-Agent's canonical project-control state without third-party dependencies."""
...
def main() -> int:
    ...
    goal_ids = {
        goal_state.get("goal_id"),
        execution_state.get("goal_id"),
        checkpoint_registry.get("goal_id"),
        iteration_state.get("goal_id"),
    }
    ...
    checkpoint_map = {
        item.get("id"): item
        for item in checkpoint_registry.get("checkpoints", [])
    ...
    if active_checkpoint not in checkpoint_map:
        ...
        checkpoint_status = checkpoint_map[active_checkpoint].get("status")
        if checkpoint_status not in {"IN_PROGRESS", "READY_FOR_REVIEW"}:
            errors.append(
                f"active checkpoint {active_checkpoint} has non-active registry status {checkpoint_status!r}"
            )
    ...
    print(
        "OK: agentic state is consistent "
        f"goal={goal_state['goal_id']} checkpoint={active_checkpoint} "
        f"iteration={active_iteration} next_wave={execution_state.get('next_wave')}"
    )
    return 0
