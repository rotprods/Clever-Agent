...
- Native follow-up: run 34709453010 builds pinned Docker successfully but the mandatory native test exits 101. The harness discarded failure output before artifact writing; diagnostics now preserve logs and timeout bytes, with 17 unit tests passing. Native runtime cause remains pending the next hosted artifact; no gate or task status changed. See `sessions/20260912-plan-integrity/diagnostics-summary.json`.
- Native cause confirmed by run 34709805284: the real sidecar self-correlated its initial hello, violating W02-05 strict supervisor identity. Its control responses also had stale correlation semantics. Sidecar now emits an unsolicited hello and correlates every response to its request; nine Python tests pass after five RED assertions. Rust validation, workflows, statuses and parity remain unchanged; require native hosted retest. See `sessions/20260912-plan-integrity/sidecar-summary.json`.
