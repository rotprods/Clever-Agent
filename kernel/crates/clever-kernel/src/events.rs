...
impl EventRouter {
    pub fn route(&mut self, event: EventEnvelope) -> Result<(), KernelError> {
        ...
        let producer = event.producer.as_ref().ok_or(KernelError::MissingField("producer"))?;
        require_text(&producer.runtime_id, "producer.runtime_id")?;
        let payload = event.payload.as_ref().ok_or(KernelError::MissingField("payload"))?;
        require_text(&payload.schema_uri, "payload.schema_uri")?;
        require_text(&payload.content_type, "payload.content_type")?;
        let classification = DataClassification::try_from(event.classification).map_err(|_| KernelError::InvalidEnum { field: "classification", value: event.classification })?;
        if classification == DataClassification::Unspecified {
            return Err(KernelError::InvalidEnum { field: "classification", value: event.classification });
        }
        if self.events.contains_key(&event.message_id) {
            return Err(KernelError::DuplicateId { kind: "event", id: event.message_id });
        }
    ...
    #[must_use]
    pub fn get(&self, message_id: &str) -> Option<&EventEnvelope> { self.events.get(message_id) }
    #[must_use]
