...
impl Display for KernelError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::UnsupportedContractMajor(major) => write!(
                formatter,
            ...
            Self::ReservedExtensionKey(key) => write!(
                formatter,
            ...
            Self::InvalidActionTransition { from, to } => write!(
                formatter,
                "invalid action receipt transition: {from} -> {to}"
            ),
            Self::FalseGreenHealth(runtime) => write!(
                formatter,
                "runtime reported false-green READY health: {runtime}"
            ),
            Self::InvalidRuntimeTransition { from, to } => write!(
                formatter,
                "invalid runtime health transition: {from} -> {to}"
            ),
            Self::AuditIntegrity { sequence } => write!(
                formatter,
                "audit hash-chain integrity failure at sequence {sequence}"
            ),
            Self::MemoryAccessDenied { memory_id, user_id } => write!(
                formatter,
                "memory access denied: {memory_id} for user {user_id}"
            ),
            Self::InvalidConfidence => {
                write!(formatter, "memory confidence must be finite and in [0,1]")
            }
        }
    }
}
