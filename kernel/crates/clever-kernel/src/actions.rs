...
impl ActionStore {
    pub fn accept(
        ...
        let risk =
            RiskClass::try_from(decision.risk_class).map_err(|_| KernelError::InvalidEnum {
        ...
        if risk == RiskClass::R4 {
            return Err(KernelError::PolicyDenied(
                "R4 actions require a dedicated hardened flow not implemented in CP02".to_owned(),
            ));
        }
