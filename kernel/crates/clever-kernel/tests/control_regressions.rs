...
fn unrelated_health_is_rejected() {
    ...
    assert!(
        peer.request_health().is_err(),
        "assertion: unrelated health cannot satisfy a request"
    );
}
#[test]
...
fn timeout_prevents_stale_connection_reuse() {
    ...
    assert!(
        peer.request_health().is_err(),
        "assertion: late health from expired request must not satisfy another request"
    );
}
#[test]
fn protocol_failure_prevents_reuse() {
    ...
    assert!(
        peer.request_health().is_err(),
        "assertion: reconnect is required after protocol ambiguity"
    );
}
#[test]
...
fn registry_invalid_tail_is_atomic() {
    ...
    assert!(
        registry.is_empty(),
        "assertion: invalid tail must not leave the valid prefix registered"
    );
}
#[test]
...
fn registry_existing_conflict_is_atomic() {
    ...
    assert_eq!(
        registry.len(),
        1,
        "existing conflict must roll back the new prefix"
    );
}
#[test]
fn registry_replay_preserves_availability() {
    ...
    assert_eq!(
        registry.get(&replay[0]).unwrap().availability,
        CapabilityAvailability::Available
    );
    assert_eq!(
        registry.get(&replay[1]).unwrap().availability,
